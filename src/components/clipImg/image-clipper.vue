    ...
    methods: {
        ...
        _initCanvas() {
            ...
            if (clipperWidth < 0 || clipperWidth > clipperClientRect.width) {
                clipperHeight = (clipperHeight / clipperWidth) * clipperClientRect.width * ratio
                clipperWidth = clipperClientRect.width * ratio
            }

            if (clipperHeight < 0 || clipperHeight > clipperClientRect.height) {
                clipperWidth = (clipperWidth / clipperHeight) * clipperClientRect.height * ratio
                clipperHeight = clipperClientRect.height * ratio
            }
        ...
        _initEvent() {
            ...
            $gesture.addEventListener('touchmove', e => {
                ...
                if (e.touches.length === 1) {
                    ...
                    // console.log(iX + f1x - scx, iY + f1y - scy)  // 移动图片边界限制 待完善
